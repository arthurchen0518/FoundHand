...
class GaussianDiffusion:
    ...
    def p_sample_loop_progressive(
        ...
        cfg_indices = list(range(50, 200))[::-1]
        ...
        for i in indices:
            if ref_conds:
                ref_cond = random.choice(ref_conds)
                model_kwargs['ref_cond'] = th.cat([ref_cond, th.zeros_like(ref_cond)])

            model_kwargs_new = model_kwargs.copy()
            # if i not in cfg_indices and 'cfg_scale' in model_kwargs:
            #     model_kwargs_new['cfg_scale'] = 1.0
